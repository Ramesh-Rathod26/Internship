...
function generate_bill(){
  ...
  $pid = $_SESSION['pid'];
  $output='';
  $query=mysqli_query($con,"select p.pid,p.AppID,p.fname,p.lname,p.doctor,p.appdate,p.apptime,p.disease,p.allergy,p.prescription,a.docFees from prestb p inner join appointmenttb a on p.AppID=a.id and p.pid = '$pid' and p.AppID = '".$_GET['AppID']."'");
  while($row = mysqli_fetch_array($query)){ 
    $output = '
    <label> Patient ID : </label>'.$row["pid"].'<br/><br/>
    <label> Appointment ID : </label>'.$row["AppID"].'<br/><br/>
    ...
    <label> Fees Due : </label>'.$row["docFees"].'<br/>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    <?php if(($row['userStatus']==1) && ($row['doctorStatus']==1))  
                        {?>
                    <a href="admin-panel.php?AppID=<?php echo $row['AppID']?>&cancel=update" onClick="return confirm('Are you sure you want to cancel this appointment ?')" title="Cancel Appointment" tooltip-placement="top" tooltip="Remove"><button class="btn btn-danger">Cancel</button></a>
                    <?php } else {
                                echo "Cancelled";
                                } ?>
                  ...
                  <td>
                    <form method="get">
                      <a href="admin-panel.php?AppID=<?php echo $row['AppID']?>">
                        ...
                        <input type="submit" onclick="alert('Bill Generated Successfully');" name="generate_bill"
                          class="btn btn-success" value="Generate Bill" />
                      </a>
                  </td>
                  </form>
